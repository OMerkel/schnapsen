"""Generate a traditional-style 52-card German SVG deck."""
...
COURT_EMOJI_FONT_SIZE = 460
COURT_EMOJI_BASELINE_Y = 695
SHEET_COLUMNS = 13
...
class Suit:
    """Color and naming metadata for one German deck suit."""
...
SUITS = (
    Suit(
        "eichel", "Eichel", "eichel", "#8a5527", "#4d2e13",
        "#b97a43", "#5a6a2f", "#7a8a4a"
    ),
    Suit(
        "blatt", "Blatt", "blatt", "#2d7f3b", "#1d5627",
        "#d8f0d1", "#2f6f38", "#4c9a5a"
    ),
    Suit(
        "herz", "Herz", "herz", "#c62839", "#8e1022",
        "#f1a1aa", "#a61d2a", "#d74a58"
    ),
    Suit(
        "schellen", "Schellen", "schellen", "#f2ca52", "#9e7300",
        "#f0c13a", "#b78908", "#d4af37"
    ),
)
...
{rdf_metadata(title)}
    <desc id="desc">Traditional-style 52-card German SVG deck.</desc>
  <defs>
...
def suit_shape_svg(suit: Suit) -> str:
    """Return the full inline SVG shape for one German suit symbol."""

    if suit.key == "eichel":
        return f'''<g fill="none" stroke-linecap="round" stroke-linejoin="round">
    <path d="M50 18 C43 9 33 7 24 10" stroke="{suit.banner}" stroke-width="4"/>
    <path d="M50 28 C37 28 28 37 28 48 C28 61 42 80 50 84 C58 80 72 61 72 48 C72 37 63 28 50 28 Z" fill="{suit.accent}" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M50 33 C44 39 42 45 42 52 C42 66 46 73 50 76 C54 73 58 66 58 52 C58 45 56 39 50 33 Z" fill="{suit.soft}" opacity="0.55"/>
    <path d="M25 38 C25 20 36 16 50 16 C64 16 75 20 75 38 C75 42 69 45 50 45 C31 45 25 42 25 38 Z" fill="{suit.figure}" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M31 35 Q35 39 39 35 M41 35 Q45 39 49 35 M51 35 Q55 39 59 35 M61 35 Q65 39 69 35" stroke="{suit.banner}" stroke-width="2"/>
</g>'''
    if suit.key == "blatt":
        return f'''<g fill="none" stroke-linecap="round" stroke-linejoin="round">
    <path d="M50 5 C72 15 88 37 86 55 C84 72 70 85 50 87 C30 85 16 72 14 55 C12 37 28 15 50 5 Z" fill="{suit.accent}" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M50 87 C49 90 48 93 48 94" stroke="{suit.dark}" stroke-width="3" stroke-linecap="round"/>
    <path d="M50 12.5 C50 36.5 50 59.5 50 80.5" stroke="{suit.soft}" stroke-width="3"/>
    <path d="M50 28.5 C55 26.5 60 24.5 62 18.5" stroke="{suit.soft}" stroke-width="2.5"/>
    <path d="M50 40.5 C60 38.5 68 35.5 70 27.5" stroke="{suit.soft}" stroke-width="2.5"/>
    <path d="M50 52.5 C39 49.5 31 43.5 26 36.5" stroke="{suit.soft}" stroke-width="2.5"/>
    <path d="M50 64.5 C39 62.5 28 55.5 21 45.5" stroke="{suit.soft}" stroke-width="2.5"/>
</g>'''
    if suit.key == "herz":
        return f'''<g fill="none" stroke-linecap="round" stroke-linejoin="round">
    <path d="M50 86 C24 66 10 51 10 33 C10 20 20 10 33 10 C41 10 47 14 50 20 C53 14 59 10 67 10 C80 10 90 20 90 33 C90 51 76 66 50 86 Z" fill="{suit.accent}" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M34 22 C27 22 22 27 22 34" stroke="{suit.soft}" stroke-width="3"/>
</g>'''
    if suit.key == "schellen":
        return f'''<g fill="none" stroke-linecap="round" stroke-linejoin="round">
    <rect x="44" y="12" width="12" height="10" rx="3" fill="{suit.soft}" stroke="{suit.dark}" stroke-width="3"/>
    <rect x="50" y="12" width="6" height="10" rx="3" fill="{suit.figure}" opacity="0.5"/>
    <rect x="44" y="12" width="12" height="10" rx="3" fill="none" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M50 22 C66 22 80 35 80 56 C80 63 74 70 66 74 C61 76 56 78 50 78 C44 78 39 76 34 74 C26 70 20 63 20 56 C20 35 34 22 50 22 Z" fill="{suit.accent}" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M50 22 C66 22 80 35 80 56 C80 63 74 70 66 74 C61 76 56 78 50 78 C50 78 50 22 50 22 Z" fill="{suit.figure}" opacity="0.4"/>
    <path d="M50 22 C66 22 80 35 80 56 C80 63 74 70 66 74 C61 76 56 78 50 78 C44 78 39 76 34 74 C26 70 20 63 20 56 C20 35 34 22 50 22 Z" fill="none" stroke="{suit.dark}" stroke-width="3"/>
    <path d="M30 57 H70" stroke="{suit.banner}" stroke-width="3"/>
    <circle cx="50" cy="66" r="6" fill="{suit.dark}"/>
    <path d="M36 83 H64" stroke="{suit.dark}" stroke-width="4"/>
    <path d="M50 76 V66" stroke="{suit.dark}" stroke-width="4"/>
</g>'''
...
def suit_symbol(suit: Suit, x: float, y: float, scale: float = 1.0, rotate: float = 0) -> str:
    """Render a German suit symbol using the source SVG path geometry."""
...
def card_svg(suit: Suit, rank: Rank, overlay_config_by_rank: dict[int, NumberOverlayConfig]) -> str:
    ...
    title = f"{rank.name} von {suit.name}"
    body = "\n".join(
        (
            card_frame(),
...
def back_svg(title: str) -> str:
  ...
  <path d="M375 384 L425 524 L550 524 L450 594 L500 734 L375 664 L250 734 L300 594 L200 524 L325 524 Z" fill="#ecd5a4" stroke="#f8eed6" stroke-width="5"/>
    <text x="375" y="950" text-anchor="middle" font-size="92" font-family="Georgia, Times New Roman, serif" letter-spacing="6" fill="#f8eed6">DEUTSCH</text>'''
    return svg_document(body, title)
...
def contact_sheet(suits: tuple[Suit, ...], ranks: tuple[Rank, ...], overlay_config_by_rank: dict[int, NumberOverlayConfig]) -> str:
    ...
    items = [
        contact_sheet_item(index, f"{rank.name} von {suit.name}", svg_text)
    ...
    inline_back_svg = inline_svg_fragment(back_svg("Cards German Double Card Back"))
    ...
    title = "Cards German Double Contact Sheet"
    return f'''<?xml version="1.0" encoding="UTF-8" standalone="no"?>
{svg_license_comment()}
<svg version="1.1"
    xmlns:dc="http://purl.org/dc/elements/1.1/"
    ...
    viewBox="0 0 {SHEET_WIDTH} {sheet_height}">
        <title id="title">{title}</title>
{rdf_metadata(title)}
        <desc id="desc">Traditional-style 52-card German SVG deck preview.</desc>
    <rect width="{SHEET_WIDTH}" height="{sheet_height}" fill="#f6efdf"/>
    <text x="{round(SHEET_WIDTH / 2)}" y="52" text-anchor="middle" font-size="32" font-family="Georgia, Times New Roman, serif" letter-spacing="4" fill="#4e3723">KARTEN DEUTSCH</text>
    <text x="{round(SHEET_WIDTH / 2)}" y="82" text-anchor="middle" font-size="15" font-family="Georgia, Times New Roman, serif" fill="#715740">Traditional-style 52-card German SVG deck preview</text>
{"\n".join(items)}
</svg>
'''
...
def generate(output_dir: Path, overlay_config_by_rank: dict[int, NumberOverlayConfig]) -> None:
    ...
    write_text(output_dir / "carte_german_double_back.svg", back_svg("Cards German Double Card Back"))
    write_text(output_dir / "carte_german_double_sheet.svg", contact_sheet(SUITS, RANKS, overlay_config_by_rank))
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate a traditional-style 52-card German SVG deck."
    )
    parser.add_argument(
        "output_dir",
        nargs="?",
